...
crew_dispatch_validate() {
  local file err copilot_profile pi_profile verified_harnesses='' typed_key typed_active=false
  ...
  if $typed_active; then
    verified_harnesses=$(fm_control_harnesses) || return 2
  fi
  err=$(jq -r --argjson pilots "{\"copilot\":$copilot_profile,\"pi\":$pi_profile}" \
    --argjson typed "$typed_active" --arg verified_harnesses "$verified_harnesses" \
    --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
    def verified($h):
      (if $typed then $verified_harnesses | split("\n")
       else ["claude","codex","opencode","pi-signed","grok","kimi","cursor","agy","muse","rovo","omp","devin"]
         + [$pilots | to_entries[] | select(.value.bootstrap) | .key]
       end) | index($h);
