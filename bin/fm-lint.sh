...
#     the full canonical set: bin/*.sh bin/backends/*.sh bin/harnesses/*.sh
#     bin/platform/*.sh tests/*.sh, with
...
fm_lint_run_backend_purity() {
  ...
  if [ "$EXPLICIT_PATHS" -eq 0 ]; then
    for path in bin/*.sh bin/backends/*.sh bin/harnesses/*.sh bin/platform/*.sh; do
      [ -f "$path" ] || continue
      purity_roots+=("$path")
    done
  else
...
# *.sh child of bin/, bin/backends/, bin/harnesses/, bin/platform/, or tests/) without the shell case
...
fm_lint_is_canonical_root() {
  ...
  case "$dir" in
    bin|bin/backends|bin/harnesses|bin/platform|tests) return 0 ;;
    *) return 1 ;;
  esac
}
