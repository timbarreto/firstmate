...
# Usage: fm-harness.sh                  print own harness: claude|codex|copilot|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|devin|unknown
...
harness_marker() {
  # Copilot's adapter additionally verifies its loader PID, preserving native
  # hook identity when Git Bash cannot see the session in its parent chain.
...
harness_process_verdict() {  # <pid>
  local pid=$1 comm args argv0 command_name harness
  ...
  if harness=$(fm_harness_identify copilot command "$command_name"); then
    printf 'comm %s\n' "$harness"; return
  elif [ "$?" -eq 2 ]; then return 2; fi
  case "$command_name" in
  ...
  if harness=$(fm_harness_identify pi command "$command_name"); then
    printf 'comm %s\n' "$harness"; return
  elif [ "$?" -eq 2 ]; then return 2; fi
  case "$command_name" in
    ...
    node*|python*)
      ...
      if harness=$(fm_harness_identify copilot interpreter "$args"); then
        printf 'args %s\n' "$harness"; return
      elif [ "$?" -eq 2 ]; then return 2; fi
      case "$args" in
      ...
      if harness=$(fm_harness_identify pi interpreter "$args"); then
        printf 'args %s\n' "$harness"; return
      elif [ "$?" -eq 2 ]; then return 2; fi
      ;;
  esac
...
harness_ancestry() {  # [<pid>]
  local pid=${1:-$$} verdict
  for _ in 1 2 3 4 5 6 7 8; do
    verdict=$(harness_process_verdict "$pid") || return 2
  ...
  while [ -n "$frontier" ] && [ "$depth" -lt 8 ]; do
    ...
    for pid in $frontier; do
      while read -r child parent; do
        ...
        if [ "$hit" = 1 ]; then
          verdict=$(harness_process_verdict "$child") || return 2
...
harness_ancestry_descent() {  # <root> [<eligible-leaf-pid>...]
  ...
  for pid in $(process_descent_path "$@"); do
    verdict=$(harness_ancestry "$pid") || return 2
    [ -n "$verdict" ] || continue
...
detect_own() {
  ...
  pin=$(supervision_primary_pin) || return 2
  [ -z "$pin" ] || { echo "$pin"; return; }
  marker=$(harness_marker) || return 2
  ancestry=$(harness_ancestry) || return 2
  if [ -z "$ancestry" ]; then
