...
VERIFIED_HARNESSES=$(fm_control_harnesses) || die "could not read verified harness capabilities"
COPILOT_PROFILE=$(fm_harness_describe copilot profile) || die "could not read Copilot profile capabilities"
PI_PROFILE=$(fm_harness_describe pi profile) || die "could not read Pi profile capabilities"
...
rules_err=$(jq -r --arg verified_harnesses "$VERIFIED_HARNESSES" \
  --argjson pilots "{\"copilot\":$COPILOT_PROFILE,\"pi\":$PI_PROFILE}" \
  --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
  def verified($h): $verified_harnesses | split("\n") | index($h);
  ...
  def effort_ok($h; $m; $e):
    ...
    elif $pilots | has($h) then ($pilots[$h].efforts | index($e)) != null
    ...
    elif $h == "pi-signed" or $h == "omp" or $h == "muse" then (["low","medium","high","xhigh","max"] | index($e)) != null
