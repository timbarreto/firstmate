...
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-session-lock-lib.sh" || return 2
# shellcheck source=bin/fm-gemini-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-gemini-lib.sh" || return 2
...
fm_agent_process_classify_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    *claude*|*codex*|copilot|copilot.exe|*opencode*|*grok*|*kimi*|*rovo*|pi|pi-signed|pi-launcher|Pi|omp) printf 'agent' ;;
