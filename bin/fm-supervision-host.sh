...
#
# Usage:
#   fm-supervision-host.sh park [--restart [--arm-owner-token TOKEN]]
#
...
# (OpenCode, omp).
# --arm-owner-token carries the adapter's per-child identity in the command
# line so native Windows retirement can find this exact host across MSYS exec.
#
  ...
  park)
    case "$#:${2:-}" in
      ...
      2:--restart|4:--restart)
        FIRST_ARM_RESTART=1
        if [ "$#" -eq 4 ]; then
          [ "$3" = --arm-owner-token ] || { echo "supervision-host: unexpected restart arguments" >&2; exit 2; }
          case "$4" in
            ''|*[!A-Za-z0-9._-]*) echo "supervision-host: invalid arm owner token" >&2; exit 2 ;;
          esac
        fi
        ;;
      *) echo "usage: fm-supervision-host.sh park [--restart [--arm-owner-token TOKEN]]" >&2; exit 2 ;;
    esac
    ;;
