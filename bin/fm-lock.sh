...
# process found by ancestry or a verified Windows session handoff, which lives as long as the
...
if [ "${1:-}" = "status" ]; then
  ...
  case "$FM_LOCK_INSPECT_STATE" in
    ...
    stale) echo "lock: stale (pid $FM_LOCK_INSPECT_PID dead or not a harness)" ;;
    *) echo "lock: unverifiable (cannot inspect harness pid $FM_LOCK_INSPECT_PID)" ;;
  esac
  exit 0
fi
