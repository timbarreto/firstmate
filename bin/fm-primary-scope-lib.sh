...
fm_primary_scope_matches() {
  local root=$1 state=$2 git_dir git_common_dir directories _extra
  if ! fm_root_is_secondmate_home "$root"; then
    directories=$(git -C "$root" rev-parse --git-dir --git-common-dir 2>/dev/null) || return 1
    {
      IFS= read -r git_dir && IFS= read -r git_common_dir && ! IFS= read -r _extra
    } <<< "$directories" || return 1
    [ -n "$git_dir" ] && [ "$git_dir" = "$git_common_dir" ] || return 1
  fi
