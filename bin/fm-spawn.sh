...
#   the new incarnation. A missing Herdr endpoint is rechecked after starting
#   its recorded server; a restored, agent-free pane can be adopted. A pane
#   proven gone is recovered only through fm-control.sh's guarded recovery
#   owner, which validates the preserved copy, lease, and competing claims
#   before creating and publishing a replacement endpoint. This launch half
...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|copilot|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|devin)
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # A restored idle pane is adoptable. Actual endpoint creation remains in the
  # control transaction, which also proves the copy's lease and unique claim.
  ...
  if [ "$RELAUNCH_STATE" = missing ] && [ "$KIND" = secondmate ]; then
  ...
  if [ "$RELAUNCH_STATE" = missing ]; then
    echo "error: task $ID's recorded endpoint is proven gone; use bin/fm-control.sh $ID relaunch --note <progress-note> so the preserved copy, lease, and task claims are verified before endpoint creation" >&2
    exit 1
  fi
  ...
  case "${POS[1]:-}" in
    ''|claude|codex|copilot|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|devin)
      ARG3=${POS[1]:-}
      ;;
    *' '*)
...
case "$HARNESS" in
  copilot|pi)
    ...
    [ "$HARNESS" != pi ] || PI_BIN=$FM_HARNESS_EXECUTABLE
    ;;
  devin)
...
model_flag_for_harness() {
  ...
  case "$harness" in
    claude|codex|opencode|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|devin)
...
spawn_prepare_private_launch_directory() { # <directory>
  local dir=$1 created=0 writable
  if (umask 077 && mkdir "$dir") 2>/dev/null; then created=1; fi
  [ ! -L "$dir" ] && [ -d "$dir" ] && [ -O "$dir" ] || return 1
  case "$(uname -s 2>/dev/null)" in
    MSYS*|MINGW*|CYGWIN*)
      if [ "$created" = 1 ]; then
        fm_private_path_native worker secure directory "$dir"
      else
        fm_private_path_native worker validate directory "$dir"
      fi
      ;;
    *)
      writable=$(find "$dir" -prune \( -perm -g=w -o -perm -o=w \) -print) || return 1
      [ -z "$writable" ] && chmod 700 "$dir"
      ;;
  esac
}
TASK_TMP="/tmp/fm-$ID"
if ! spawn_prepare_private_launch_directory "$TASK_TMP"; then
  echo "error: task temp root $TASK_TMP is not a private directory owned by this user; refusing to stage the launch command there; inspect and remove it, then retry" >&2
  exit 1
fi
...
preserve_relaunch_meta() {
  awk -F= '
    BEGIN {
      split("window endpoint_task_id worktree project harness kind mode yolo branch tasktmp model effort account account_provider busy_gen spawn_gen launch_status traceparent backend herdr_session herdr_workspace_id herdr_tab_id herdr_pane_id zellij_session zellij_tab_id zellij_pane_id orca_worktree_id terminal cmux_workspace_id cmux_surface_id home projects control_relaunch_tx", keys, " ")
...
case "$HARNESS" in
  claude|codex|opencode|pi-signed|grok|kimi|gemini|muse|rovo|omp|agy|devin)
...
COMPACT_ADVISER_COMMAND="export COMPACT_ADVISER_DISABLE=1"
if [ "$WINDOWS_HERDR_POWERSHELL" = 1 ]; then
  COMPACT_ADVISER_COMMAND=$(fm_backend_herdr_windows_set_environment_command COMPACT_ADVISER_DISABLE 1) || {
    echo "error: could not compose the native Windows compact-adviser assignment for $ID" >&2
    exit 1
  }
fi
spawn_send_text_line "$T" "$COMPACT_ADVISER_COMMAND"
if [ "$LAVISH_AXI_HOST_CONFIG_PRESENT" = 1 ]; then
  LAVISH_HOST_COMMAND="export LAVISH_AXI_HOST=$(shell_quote "$LAVISH_AXI_HOST")"
  if [ "$WINDOWS_HERDR_POWERSHELL" = 1 ]; then
    LAVISH_HOST_COMMAND=$(fm_backend_herdr_windows_set_environment_command LAVISH_AXI_HOST "$LAVISH_AXI_HOST") || {
      echo "error: could not compose the native Windows Lavish host assignment for $ID" >&2
      exit 1
    }
  fi
  spawn_send_text_line "$T" "$LAVISH_HOST_COMMAND"
fi
# Mark ship and scout panes so the runner refuses their primary checkout.
# Secondmates run their own homes rather than an assigned task worktree.
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  TASK_ID_COMMAND="export FM_TASK_ID=$ID"
  if [ "$WINDOWS_HERDR_POWERSHELL" = 1 ]; then
    TASK_ID_COMMAND=$(fm_backend_herdr_windows_set_environment_command FM_TASK_ID "$ID") || {
      echo "error: could not compose the native Windows FM_TASK_ID assignment for $ID" >&2
      exit 1
    }
  fi
  spawn_send_text_line "$T" "$TASK_ID_COMMAND"
fi
...
if ! spawn_prepare_private_launch_directory "$LAUNCH_DIR"; then
  echo "error: task launch directory $LAUNCH_DIR is not a private directory owned by this user; refusing to stage the launch command there; inspect and remove it, then retry" >&2
  exit 1
fi
...
if ! (umask 077 && printf '%s\n' "$LAUNCH" >"$LAUNCH_STAGE" &&
  fm_pr_private_file_secure "$LAUNCH_STAGE" 600 && mv -f "$LAUNCH_STAGE" "$LAUNCH_FILE"); then
  rm -f "$LAUNCH_STAGE"
  echo "error: could not stage the launch command at $LAUNCH_FILE" >&2
  exit 1
fi
LAUNCH_DELIVERY_COMMAND=". $(shell_quote "$LAUNCH_FILE")"
if [ "$WINDOWS_HERDR_POWERSHELL" = 1 ]; then
  LAUNCH_DELIVERY_COMMAND=$(fm_backend_herdr_windows_bash_script_command "$LAUNCH_FILE") || {
