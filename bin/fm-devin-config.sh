...
# Output: <state-dir>/<task-id>.devin-config.json, atomically replaced with
# mode 600 on POSIX or a verified private native Windows DACL.
...
. "$SCRIPT_DIR/fm-private-path-lib.sh"
...
case "${OSTYPE:-}" in
  msys*|mingw*|cygwin*)
    fm_private_path_native pr secure file "$temp" || {
      echo "error: could not secure private Devin config: $temp" >&2
      exit 1
    }
    ;;
esac
