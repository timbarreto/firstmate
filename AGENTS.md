...
The verified harnesses are `claude`, `codex`, `copilot`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `gemini` and `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
