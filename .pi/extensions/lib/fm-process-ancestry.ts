...
function nativePidAlive(pid: string): boolean {
...
let cachedWindowsAncestry: Map<string, string> | null = null;
let cachedWindowsProcessPid = "";

function windowsAncestry(maxDepth: number): Map<string, string> {
...
export function isPidInCurrentAncestry(pid: string, maxDepth = 8): boolean {
...
export function signalWatchArmProcess(pid: number | undefined, ownerToken = ""): boolean {
  if (!Number.isSafeInteger(pid) || (pid ?? 0) < 2) return false;
  if (process.platform !== "win32") {
    try {
...
export function terminateWatchArmProcessTree(pid: number | undefined, ownerToken = ""): boolean {
  if (!Number.isSafeInteger(pid) || (pid ?? 0) < 2) return false;
  if (process.platform === "win32") {
    const result = spawnSync(
      windowsPowerShell,
