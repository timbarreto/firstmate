...
for name in devin devin-helper; do
  fm_test_make_symlink /bin/bash "$TMP_ROOT/names/$name" || fail "could not create $name identity fixture"
done
...
case "${OSTYPE:-}" in
  msys*|mingw*|cygwin*)
    ...
    fm_private_path_native pr validate any "$config" || fail 'config is not private on Windows'
    ;;
  *)
    [ -n "$(find "$config" -prune -type f -perm 0600 -print)" ] || fail 'config is not mode 600'
    ;;
esac
