...
# Only the explicitly marked handoff fixtures inject attachment; pane and
# focus mutations still execute against the real guarded lab.
if [ "${1:-} ${2:-} ${3:-}" = "terminal title clear" ] \
   && [ -n "${FM_TEST_HERDR_FOREGROUND_REASON:-}" ]; then
  printf '{"result":{"reason":"%s"}}\n' "$FM_TEST_HERDR_FOREGROUND_REASON"
  exit 0
fi
focus_snapshot() {
...
write_ship_brief "$HOME_DIR" active-cleanup-safe 'Projection active cleanup safe-focus fixture.'
write_ship_brief "$HOME_DIR" active-cleanup-deferred 'Projection active cleanup deferred fixture.'
...
FM_TEST_HERDR_FOREGROUND_REASON=cleared teardown_task active-cleanup-safe "$HOME_DIR" \
...
pass "real Herdr lab with injected live-viewer response: cleanup restores the exact parent Firstmate tab and completes"
...
FM_TEST_HERDR_FOREGROUND_REASON=cleared teardown_task active-cleanup-deferred "$HOME_DIR" \
...
pass "real Herdr lab with injected live-viewer response: no safe focus target yields an idempotent action-free defer"
