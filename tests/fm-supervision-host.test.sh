...
fm_test_make_symlink /bin/bash "$FAKEBIN/claude" || fail "could not create the host-owner identity fixture"
...
test_first_cycle_status_streams_and_owner_options_reach_it() {
  ...
  start_host "$home" --restart --arm-owner-token "host-${BASHPID:-$$}"
...
test_owner_token_arguments_refuse_malformed_input() {
  local token out rc home="$TMP_ROOT/invalid-options"
  mkdir -p "$home"
  for token in '' '../other' 'token with spaces'; do
    out=$(FM_HOME="$home" "$HOST" park --restart --arm-owner-token "$token" 2>&1); rc=$?
    expect_code 2 "$rc" "malformed owner token should refuse"
    assert_contains "$out" 'invalid arm owner token' "owner-token refusal should identify the malformed argument"
  done
  out=$(FM_HOME="$home" "$HOST" park --restart --not-owner token 2>&1); rc=$?
  expect_code 2 "$rc" "unknown restart arguments should refuse"
  assert_contains "$out" 'unexpected restart arguments' "unknown restart argument should be explicit"
  out=$(FM_HOME="$home" "$HOST" park --arm-owner-token token 2>&1); rc=$?
  expect_code 2 "$rc" "owner tokens without restart should refuse"
  pass "host owner-token parsing refuses malformed and unsupported arguments"
}
