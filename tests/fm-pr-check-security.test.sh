...
run_watcher_bounded() {
  ...
  FM_TEST_WATCH_TIMEOUT=$watch_timeout \
    perl -MPOSIX=WNOHANG -MTime::HiRes=time,sleep -e 'my $pause=shift; my $left=$ENV{FM_TEST_WATCH_TIMEOUT}; my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } my $last=time; while (waitpid($pid, WNOHANG) == 0) { my $now=time; $left -= $now - $last unless length $pause && -e $pause; $last=$now; if ($left <= 0) { kill "TERM", $pid; waitpid $pid, 0; exit 124 } sleep 0.02 } exit($? >> 8)' \
...
SH
    for check in custom z-drain-witness; do
      fm_pr_private_file_secure "$state/$check.check.sh" 700 \
        || fail "could not secure the $backend returned-descendant $check check"
...
shift_registration_device() {  # <state> <id> [both|data|check]
  ...
  fm_pr_private_file_secure "$tmp" 600 || fail "could not secure a shifted registration"
...
test_device_rerecord_preserves_private_policy() (
  local dir state original control_lock publish_lock
  dir=$(make_case device-rerecord-private-policy)
  state="$dir/home/state"
  make_windows_directory_inheritance_broad "$state" || fail "could not prepare inherited ACL fixture"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/1
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/1
  ...
  publish_lock="$state/.pr-poll-publish-task-a.lock"
  fm_lock_acquire_wait "$control_lock" || fail "control lock unavailable"
  fm_lock_acquire_wait "$publish_lock" || fail "publication lock unavailable"
  trap 'fm_lock_release "$publish_lock"; fm_lock_release "$control_lock"' EXIT
  fm_pr_poll_registration_rerecord_device "$state" task-a "$POLL" \
    || fail "private re-record failed"
  [ "$(fm_pr_sha256 "$state/task-a.pr-poll-registration")" = "$original" ] \
    || fail "device repair changed registration content beyond identities"
  fm_pr_poll_artifacts_valid "$state" task-a "$POLL" \
    || fail "device repair did not restore private strict authentication"
  ! fm_pr_poll_registration_device_shifted "$state" task-a "$POLL" \
    || fail "repaired poll still claimed device drift"
  printf '# tampered\n' >> "$state/task-a.check.sh"
  ! fm_pr_poll_registration_rerecord_device "$state" task-a "$POLL" \
    || fail "device repair accepted altered check bytes"
  pass "device repair preserves native/POSIX privacy, exact content, and subsequent tamper refusal"
)
...
SH
  ...
  fm_pr_private_file_valid "$state/task-a.pr-poll-registration" 600 "$(fm_pr_file_device "$state")" \
    || fail "re-recorded registration is not private"
...
if [ "$last" = "$FM_TEST_FOREIGN_PATH" ]; then
  case " $* " in
    *" %d %i %h %a "*)
      facts=$("$FM_TEST_REAL_STAT" "$@") || exit $?
      printf '%s %s\n' "$FM_TEST_FOREIGN_DEVICE" "${facts#* }"
      exit 0
      ;;
