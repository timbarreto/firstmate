...
if (!hosts.every((row) => / args=park --restart --arm-owner-token opencode-[A-Za-z0-9._-]+ primary=opencode /.test(row))) throw new Error(`the host must run as 'park --restart' with the opencode pin and owned-child token: ${hosts.join(" | ")}`);
const ownerTokens = hosts.map((row) => row.match(/--arm-owner-token ([A-Za-z0-9._-]+)/)?.[1]);
if (new Set(ownerTokens).size !== hosts.length) throw new Error(`successor hosts reused an owner token: ${hosts.join(" | ")}`);
