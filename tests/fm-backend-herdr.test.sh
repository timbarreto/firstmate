...
test_registered_copilot_foreground_process_stays_alive() {
  local name out body
  for name in copilot copilot.exe; do
    body=$(printf '{"result":{"type":"pane_process_info","process_info":{"pane_id":"w1:p2","shell_pid":4242,"foreground_process_group_id":4243,"foreground_processes":[{"pid":4243,"name":"%s","argv0":"%s","argv":["%s"],"cmdline":"%s"}]}}}' \
      "$name" "$name" "$name" "$name")
    out=$(stale_registration_case "live-$name" idle "$body")
    [ "$out" = "live alive refused" ] \
      || fail "a $name foreground process must stay live/alive and refuse husk closing, got '$out'"
  done
