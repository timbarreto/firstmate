...
test_crew_dispatch_validation() {
  local label body expect mode key case_dir fakebin out child_env n
  n=0
  while IFS='^' read -r label body mode expect; do
    ...
    add_real_jq "$fakebin"
    key=test-key
    case "$mode" in legacy-*) key=''; mode=${mode#legacy-} ;; esac
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
      TYPESAFE_API_KEY="$key" FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      exact)
...
pilot null model is malformed^{"default":{"harness":"pi","model":null,"effort":"ultra"}}^legacy-exact^CREW_DISPATCH: invalid config/crew-dispatch.json - default profile model and effort must be non-empty strings when present
pilot null effort is malformed^{"default":{"harness":"copilot","effort":null}}^legacy-exact^CREW_DISPATCH: invalid config/crew-dispatch.json - default profile model and effort must be non-empty strings when present
...
gemini control support does not admit dispatch^{"default":{"harness":"gemini"}}^legacy-exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: gemini
...
provider-less multi-provider profile remains accepted without opt-in^{"rules":[{"when":"cross-provider work","use":{"harness":"opencode","model":"anthropic/claude-sonnet-4-5"}}],"default":{"harness":"pi","model":"anthropic/claude-sonnet-5"}}^legacy-empty^
