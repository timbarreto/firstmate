...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u COPILOT_CLI -u COPILOT_AGENT_SESSION_ID -u COPILOT_LOADER_PID '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u COPILOT_CLI -u COPILOT_AGENT_SESSION_ID -u COPILOT_LOADER_PID '$fallback' --auto" ] \
...
test_kimi_readiness_gate_precedes_pointer() {
  ...
  # The side-band producer must observe the final rollback outcome, not a
  # provisional endpoint captured before launch readiness was known.
  [ -d "$HOME_DIR/state/.home-summary-refresh.request" ] \
    || fail "kimi readiness failure lost its deferred summary reconciliation request"
