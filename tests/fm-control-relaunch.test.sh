...
# The control deadline uses a real watchdog; do not turn its sleep into a no-op.
exec /bin/sleep "$@"
SH
  chmod +x "$fb/sleep"
}
...
case "${1:-} ${2:-}" in
  'pane get')
    if [ "${3:-}" = seedpane ] && [ ! -f "$D/herdr-seed-closed" ]; then
      printf '{"result":{"pane":{"pane_id":"seedpane","tab_id":"seedtab","workspace_id":"wsnew"}}}\n'
  ...
  'pane send-text')
    ...
    exit 0 ;;
  'workspace list')
    printf '{"result":{"workspaces":[{"workspace_id":"ws1","active_tab_id":"tab1","focused":true}]}}\n'
    exit 0 ;;
  'session list')
    jq -n --arg name "$(cat "$D/herdr-session")" --arg socket "$D/herdr.sock" \
  ...
  'workspace create')
    ...
    printf '{"result":{"workspace":{"workspace_id":"wsnew"},"tab":{"tab_id":"seedtab"},"root_pane":{"pane_id":"seedpane"}}}\n'
    exit 0 ;;
  'tab list')
    if [ "${4:-}" = ws1 ]; then
      printf '{"result":{"tabs":[{"tab_id":"tab1","focused":true}]}}\n'
    elif [ -f "$D/herdr-seed-closed" ]; then
      printf '{"result":{"tabs":[{"tab_id":"tabnew","label":"worker"}]}}\n'
    else
      printf '{"result":{"tabs":[{"tab_id":"seedtab","label":"1"},{"tab_id":"tabnew","label":"worker"}]}}\n'
    fi
    exit 0 ;;
  'pane list')
    if [ -f "$D/herdr-seed-closed" ]; then
      printf '{"result":{"panes":[{"pane_id":"%%9","tab_id":"tabnew"}]}}\n'
    else
      printf '{"result":{"panes":[{"pane_id":"seedpane","tab_id":"seedtab"},{"pane_id":"%%9","tab_id":"tabnew"}]}}\n'
    fi
    exit 0 ;;
  'terminal title')
...
add_herdr_ship_task() {  # <case-dir> <id> [session] [surviving-pane]
  ...
  jq -n --arg path "$wt" --arg task "$id" \
    '[{path:$path,status:"leased",lease_id:"fixture-lease",lease_holder:$task,processes:[]}]' \
    > "$dir/fake/pool.json"
  cat > "$dir/fakebin/treehouse" <<'SH'
#!/usr/bin/env bash
[ "$*" = 'status --json' ] || exit 2
cat "$FM_FAKE_DIR/pool.json"
SH
...
EOF
  ...
  printf '%s' "$wt" > "$dir/fake/cwd"
  printf '%s' "$ses" > "$dir/fake/herdr-session"
...
fm_backend_herdr_presentation_lock_namespace() {
  printf '%s/herdr-locks' "$FM_FAKE_DIR"
}
SH
  mkdir -m 700 "$namespace" || fail "could not create the fixture lock namespace"
  case "$(uname -s)" in
    MSYS*|MINGW*|CYGWIN*)
      # shellcheck source=bin/fm-private-path-lib.sh
      . "$ROOT/bin/fm-private-path-lib.sh"
      fm_private_path_native worker secure directory "$namespace" \
        || fail "could not secure the fixture lock namespace"
      ;;
  esac
  CONTROL="$code/bin/fm-control.sh" FM_ROOT_OVERRIDE="$code" run_control "$dir" "$@"
)
...
test_herdr_rebind_stays_in_the_recorded_session() {
  ...
  out=$(run_control "$dir" rl73 relaunch --note "Recover in the recorded session.") || rc=$?
  log=$(cat "$dir/fake/herdr-log")
...
test_herdr_reclaim_keeps_the_task_whole() {
  ...
  out=$(run_herdr_control "$dir" rl75 relaunch --note "the pane was destroyed; pick the work back up") || rc=$?
  ...
  [ "$(git -C "$dir/wt" rev-parse --abbrev-ref HEAD)" = "fm/rl75" ] \
  ...
  [ "$(journal_field "$dir" rl75 exit_result)" = 'endpoint-gone rl75 (missing endpoint recovered)' ] \
...
test_herdr_rebind_failure_from_a_plain_shell_names_the_real_cause() {
  ...
  # The record names `fmlab`; the recovery transaction creates only in that
  # session and must report a creation failure without inventing a seat mismatch.
  ...
  out=$(run_herdr_control "$dir" rl77 relaunch --note "Recover the preserved task."); rc=$?
...
test_spawn_relaunch_defers_missing_endpoint_to_control() {
  local dir out rc=0
  herdr_case_or_skip gone-herdr-direct rl78 fmlab '%none' || {
    echo "skip - herdr reclaim needs jq (the herdr adapter parses JSON with it)"
    return 0
  }
  dir=$HERDR_CASE_DIR
  cp "$dir/home/state/rl78.meta" "$dir/before.meta"
  out=$(run_spawn "$dir" rl78 --relaunch --harness claude) || rc=$?
  expect_code 1 "$rc" "direct spawn must not bypass the guarded recovery owner"
  assert_contains "$out" "bin/fm-control.sh rl78 relaunch" "the refusal must name the recovery owner"
  assert_not_contains "$(cat "$dir/fake/herdr-log")" "workspace create" "direct spawn bypassed lease validation"
  cmp -s "$dir/before.meta" "$dir/home/state/rl78.meta" || fail "direct spawn rewrote the unrecovered binding"
  pass "relaunch: direct spawn leaves endpoint creation to guarded control recovery"
}
