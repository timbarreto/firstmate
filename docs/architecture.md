...
PR registration and watcher snapshot capture serialize through the publication contract in `bin/fm-pr-check.sh`'s header, so a busy replacement is deferred rather than reported as an unauthenticated check.
...
[`tests/fm-crew-state.test.sh`](../tests/fm-crew-state.test.sh) covers run selection, including capped-rerun interleavings and bounded inventory refresh; its [capture provenance and live-evidence limits](../tests/captures/no-mistakes-v1.70.1/README.md) distinguish recorded inputs from composed scenarios.
...
Each home atomically publishes that bounded home summary with freshness epoch metadata at `state/home-summary.json` after a locked session start, a watcher-observed status change, task teardown, and on a recurring live-watcher cadence; task spawn requests a coalesced refresh through that existing watcher rather than waiting for reporting before delivery.
`bin/fm-home-summary-refresh.sh` owns request cohorts, retry, and publication mechanics, and the prior complete ledger retains its original freshness timestamp until replacement.
A locked session start launches that publication off the blocking digest so a slow summary cannot consume the runtime bound.
