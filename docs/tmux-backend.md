...
It classifies recognized Claude, Codex, Copilot, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, and omp process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
