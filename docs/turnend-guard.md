...
- GitHub Copilot CLI registers shell-completion `notification` and `agentStop` hooks in `.github/hooks/firstmate.json`.
  ...
  Copilot also imports `.claude/settings.json`, preferring its platform-specific `bash` or `powershell` hook field over `command`.
  Every tracked Claude entry supplies successful no-ops in those Copilot dispatch fields, leaving `command` intact for Claude and leaving Copilot's own `.github/hooks/` protections active.
  Without that separation, the wildcard Claude pre-tool entry is parsed as PowerShell on Windows and can reject even file reads and skill loading before a script-level host guard could run.
...
`tests/fm-copilot-harness.test.sh` covers Copilot's native Windows identity bridge, per-event session binding when native hooks omit the environment marker, session-start context, native blocked response, nonblocking stop backstop, shell-completion notification routing, forced-continuation reset and ceiling, worker semantic lifecycle, and imported Claude entries remaining inert for Copilot without disarming Claude's commands.
`tests/fm-copilot-primary-live-e2e.test.sh` uses an isolated Windows Herdr session and local deterministic provider to verify the real Stop repair, confirmed native asynchronous watcher startup, responsiveness during monitoring, completion handling, rearming, and literal agent exit.
`FM_COPILOT_HOOKS_LIVE_E2E=1 tests/fm-copilot-hooks-live-e2e.test.sh` proves repository hook discovery, asynchronous shell-completion context injection, and successful skill/file reads with both tracked configurations present while Copilot's own pre-tool denial remains enforceable.
