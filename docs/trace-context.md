...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `copilot`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, `gemini`, and `muse`, plus Secondmate spawns across that same set except the deliberately crewmate-only `gemini` and `muse` adapters.
