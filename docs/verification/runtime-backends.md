...
In the direct-child Pi fixture on Herdr 0.9.0, `herdr agent get` reflected process liveness; its registration did not persist merely because the pane and parent shell did.
...
`herdr pane get` `.agent_status` can still read `idle` after the occupant is gone.
The current classifier combines `agent get` with `pane process-info` and descendant checks; neither a pane status nor a registration alone establishes liveness.
The direct-child result does not generalize to the nested worktree-shell shape in [Stale agent registration](#stale-agent-registration).
