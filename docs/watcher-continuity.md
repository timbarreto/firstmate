...
On Pi, omp, OpenCode, Copilot, Cursor, and Claude primaries, one component owns re-arming the watcher.
...
| Copilot | Tracked asynchronous shell task and `.github/hooks/firstmate.json` completion hook |
...
On a supported non-Pi primary, a home opted into the supervision host also changes what the owner runs; see [Supervision host](#supervision-host).
...
On Claude, Cursor, OpenCode, omp, Grok, and Codex primaries, a home opted into the supervision host runs `bin/fm-supervision-host.sh` in place of the arm its re-arm owner would start.
Copilot keeps its tracked asynchronous shell path, and Pi keeps its native branch extension.
...
- Copilot depends on tracked asynchronous shell completion notifications.
